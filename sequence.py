...
class PhaseTrack:
    """ Phase Tracking Class used for phase stability within chunk.
...
class MetaChunk:
    """ Data structure that carries all metadata for a chunk."""
...
class Chunk:
    ...
    def __init__(self, phase, dur, off, meta, prevChunk=None, samplingRate=50*10**9):
        ...
        :samplingRate: Sampling rate of AWG
        """
        self.meta = meta
        self.groomedOutputs = []
        self.trackedPhases = []
        ...
        if prevChunk is not None:
            if prevChunk.trackedPhases is not None:
            ...
            if np.size(self.trackedPhases) > 0:
                ...
                for tPh in self.trackedPhases:
                    if tPh.hasFreq(ph):  # if yes grab the old angle
                        hasPhase = 1
                        prevAngle = prevAngle+tPh.getAngle(
                            np.size(self.outputs)+offLength
                        )
